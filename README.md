...
✅ **Pitch Image Agent** - Always-on agent that background-renders an incident-hero cold-open image via **ElBruno.Text2Image.Foundry** (`IImageGenerator`, **GPT-Image-2 / Azure OpenAI**) when GPT-Image-2 credentials are configured, served at `/api/pitch/hero-image`, displayed in the chat UI, and triggerable on demand via the **"Generate an incident-response image"** sample prompt (`/api/pitch/generate-image`)  
